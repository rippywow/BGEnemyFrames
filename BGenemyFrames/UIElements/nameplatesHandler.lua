	...
	-------------------------------------------------------------------------------
	local TEXTURE = [[Interface\AddOns\BGenemyFrames\globals\resources\barTexture.tga]]
	...
	local addBuffs = function(plate, name)
		...
		if not plate.buffs then			
			...
			for i = 1, maxBuffs do
				...
				if i == 1 then
					plate.buffs[i]:SetPoint('BOTTOMLEFT', plate, 'TOPLEFT', -52.5, 14)
				else
				...
				plate.buffs[i].duration:SetFont(STANDARD_TEXT_FONT, 12, 'OUTLINE')
